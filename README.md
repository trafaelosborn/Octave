...
- Inventories local source folders, preserves manual primary/secondary/archive tags in `.octave/source-inventory.json`, and builds reusable evidence maps plus source-grounded briefs over those files.
- Checks active drafts against saved evidence maps and citation reports, writing claim-check reports under `.octave/claim-checks/`.
...
```text
app/
  api/         workspace, document, chat, compile, search, source, evidence-map, claim-check, citation, submission, and run routes
  components/  editor workstation, chat, PDF, navigation, and revision review
  lib/         workspace registry, context state, diffs, outline, source/evidence inventory, citation scan, and citation check summaries
desktop/
...
The Sources rail scans source-like folders such as `sources/`, `primary/`, `secondary/`, `archive/`, and `data/`, infers primary/secondary/archive roles, and lets the researcher correct them. The saved `.octave/source-inventory.json` becomes a machine-readable evidence shelf for source-grounded briefs. Evidence maps are paired JSON and Markdown artifacts under `.octave/evidence-maps/` that preserve source snapshots, candidate passages, extraction warnings, and review fields for direct support, inference, and missing sources. Claim checks compare the active draft against saved evidence-map passages and the latest citation-check report, then write paired JSON/Markdown review queues under `.octave/claim-checks/`.
